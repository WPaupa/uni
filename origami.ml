...
let kolko (xs,ys) r (x,y) =
  if r>1e150 then 
    let xs = xs /. r and ys = ys /. r and x = x /. r and y = y /. r in 
    if ((x-.xs)*.(x-.xs)) +. ((y-.ys)*.(y-.ys)) <= 1. then 1 else 0
  else if ((x-.xs)*.(x-.xs)) +. ((y-.ys)*.(y-.ys)) <= r *. r then 1 else 0 ;;

let cross (ux,uy) (vx,vy) = (uy*.vx)-.(ux*.vy);;
let scalar (ux,uy) (vx,vy) = (ux*.vx)+.(uy*.vy);; 
