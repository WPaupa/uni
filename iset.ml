...
let cmp (a,b) (c,d) = if b<(minus c 1) then -1 else if a>(plus d 1) then 1 else 0 
...
(*gotowe - przepisane z pseta*)
let make l (ka,kb) r = Node (l, (ka,kb), r, max (height l) (height r) + 1, plus (plus (elementy r)  (elementy l))  (plus (minus kb ka) 1))
  ...
  if hl > hr + 2 then
    ...
    | Node (rl, rk, rr, _, _) ->
        if height rr >= height rl then make (make l k rl) rk rr
          ...
          | Empty -> assert false)
    | Empty -> assert false
  else Node (l, k, r, max hl hr + 1, plus (plus (elementy r)  (elementy l))  (plus (minus (snd k) (fst k)) 1))
...
let rec add_pset x = function
  | Node (l, k, r, h, _) ->
      ...
      if c = 0 then 
        ...
        Node (l, (ka, kb), r, h, plus (plus (elementy r)  (elementy l))  (plus (minus kb ka) 1))
      else if c < 0 then
        ...
        bal l k nr
  | Empty -> Node (Empty, x, Empty, 1, plus (minus (snd x) (fst x)) 1)
...
let split x set =
  let rec loop x = function
    ...
    | Node (l, v, r, _, _) ->
        let c = comp x v in
        if c = 0 then 
        	let p1 = if (fst v)< x then add_one (fst v,(minus x 1)) l else l  
        	and p2 = if x<(snd v) then add_one (plus x 1,snd v) r else r in
        	(p1, true, p2)
        else if c < 0 then
...
let below n set = 
  if (second (split n set))
    then 1 + elementy (first (split n set))
